...
\section{The Theory}
We will now describe a variant of Ackermann Set Theory \cite{ackermann} we denote as \textsf{AST+NWF}. \textsf{AST+NWF} is formulated in first-order logic with equality and with a constant $V$ which is interpreted as the set universe, and a binary relation $\in$ which is interpreted as the usual membership relation. Here are the axioms of $\textsf{AST+NWF}(V)$:
...
\begin{enumerate}
    ...
    \item Axiom of Supercompleteness $\textsf{SC}(V)$: $$\forall x, y (x \in V \wedge (y \in x \vee y \subseteq x) \rightarrow y \in V)$$ where $\subseteq$ is the usual subset relation, defined as $x \subseteq y \leftrightarrow \forall z(z \in x \rightarrow z \in y).$
    \item Ackermann Schema $\textsf{Ack}(V)$: Let $\phi(y, z_0, \ldots, z_{n-1})$ be a unary first-order formula where all the constants $z_0, \ldots, z_{n-1} \in V$ and $\ne V$. Then $$(\forall y(\phi(y, \ldots) \rightarrow y \in V)) \rightarrow \exists x(x \in V \wedge \forall y(y \in x \leftrightarrow \phi(y, \ldots))).$$
...
$$(\bigwedge_i \forall y_i( \phi(y_i, \ldots_i) \rightarrow y_i \in V)) \rightarrow \\ (\forall y(\phi(y, \ldots) \rightarrow y \in V)),$$
then
\begin{align*}
...
\section{Motivations}
We have at least two motivations for the ``design" of the theory.\\
...
Another motivation is to easily extend to multiple universes as desired: $V_0, V_1, \ldots$ such that $V_0 \in V_1 \in \ldots$ and $\textsf{AST-NWF}(V_0)$ and $\textsf{AST-NWF}(V_1)$ and $\ldots.$ Note that \textsf{SC} and a contradiction from \textsf{Ack} should imply that $V_0 \subset V_1 \subset \ldots.$ Universes can be thought of as those that are ``beyond construction", and we need to keep track of those and keep all our discourse inside of those. This is also why we restricted \textsf{Ext} to $V$.
