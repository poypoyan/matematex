...
where all those instances of $\phi$\footnote{Yes, this is a recursive definition!} are also \textit{defined}, and in the last ellipsis, elements and subsets of any $x_i$ \textit{should not occur}. Then if
...
On the contrary, consider the Axiom of Choice (AC), a well-known axiom independent to \textsf{ZF}. The sets it constructs are not unique, hence it is said to postulate existence of sets without defining it, unlike other \textsf{ZF} (set construction) axioms \cite[Chapter 5]{jech}. However, if we just have the ``power" to construct sentences with lengths of any ordinal, we can then ``deduce'' AC from the axiom schema, and hence we can see that the sets it constructs are unique! To see this, let $S$ be a set of sets, then an appropriate $\phi$ for AC would simply be $$y = a_0 \vee y = a_1 \vee y = a_2 \vee \ldots$$ where $a_0 \in s_0, a_1 \in s_1, a_2 \in s_2, \ldots$ (the $a_i$ are our actual choices) and $S=\{s_0, s_1, s_2, \ldots\}$. Nevertheless, stronger axioms like Choice can be added to our theory via relativization to $V$ wherein every instance of $\forall x \varphi$ in the statement is replaced by $\forall x (x \in V \rightarrow \varphi)$ and every instance of $\exists x \varphi$ is replaced by $\exists x (x \in V \wedge \varphi)$.
...
For example, to construct a set such that $x=\{x, a\}$ where $a \in V$, $\phi(y, a)$ is defined as $$\forall x_0 (\forall y_0 (y_0 \in x_0 \leftrightarrow \phi(y_0, a)) \rightarrow y = x_0 \vee y = a).$$ Notice that this satisfies the prescribed form for $\phi$. This is motivated by what we'll call the \textit{Opacity Principle}: elements and subsets of an NWF set should not be used to build that set, for two reasons: 1) to prevent constructing properties ``so loose" that well-founded sets can satisfy it (i.e., $\omega$ is a set such that $\bigcup x = x$), and 2) to prevent inconsistency (i.e., a set such that $2^x = x$ contradicts Cantor's theorem). While definitely a constraint, ``here be dragons" is apt for what lies beyond this. Next, the statement ``all those instances of $\phi$ are also \textit{defined}" means that $x(\emptyset) = \{x(\{\emptyset\}), x(\emptyset)\}$ for example, will not be defined unless $x(\{\emptyset\})$ is defined first.
