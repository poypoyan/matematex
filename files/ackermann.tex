...
\begin{enumerate}
    ...
    \item \textbf{Non-well-founded} Ackermann Schema $\textsf{NWFAck}(V)$: Let $p$ be a \textit{new} $(m+1)$-ary predicate. Let $$\phi(y, z_0, \ldots, z_{m-1}, a_0, \ldots, a_{n-1})$$ be any unary first-order formula where all the free variables $z_0, \ldots, z_{m-1}, a_0, \ldots, a_{n-1} \in V$ and $\ne V$, and there are $m$ instances of $p$ in $\phi$:
\begin{align*}
&p(y, 0, 0, \ldots, 0, z_0) \\ &p(y, 1, 0, \ldots, 0, z_1) \\ &p(y, 1, 1, \ldots, 0, z_2) \\ &\vdots \\ &p(y, 1, \ldots,1, 1, z_{m-1})
\end{align*}
where we define $0 = \emptyset$ and $1 = \{0\}$. Then if
$$\forall x(\bigwedge_{i=0}^{m-1}{p(x, [i > 0], [i > 1], \ldots, z_i)}\rightarrow x \in V) \rightarrow \\ (SC(V) \rightarrow \forall y(\phi(y, \ldots) \rightarrow y \in V))$$
where $[\cdot]$ is the Iverson bracket, then
\begin{align*}
\bigwedge_{i=0}^{m-1}\Big(\exists!x(p(x, [i > 0], [i > 1], \ldots, z_i)) \wedge \forall x\Big(p(x, [i > 0], [i > 1], \ldots, z_i) \rightarrow \\ \Big(x \in V \\ \wedge\ \forall c_0, \ldots, c_{m-1} (p(x, c_0, \ldots, c_{m-1}) \rightarrow \bigwedge_{j=0}^{m-2} {c_j = [i > j]} \wedge c_{m-1} = z_i) \\ \wedge \bigwedge_{j=0}^{m-2} p_j^{-1}([i > j], x) \wedge p_{m-1}^{-1}(z_i, x)\Big)\Big)\Big)
\end{align*}
where $p_j^{-1}$ are \textit{new} binary predicates that ``retrieve" the parameters of $p$,
and
$$\forall x (\forall y (y \in x \leftrightarrow \phi(y, \ldots)) \leftrightarrow p(x, z_0, \ldots, z_{m-1})).$$
\end{enumerate}
...
Lastly, still in NWF Ackermann Schema, the motivation behind restricting to $m$ instances of $p$ in $\phi$ and the usage of $z_0, \ldots, z_{m-1}$ in $p$ in the final clause is also the above definition of ordered pair. We are still not fully satisfied with the axiom schema, but we hope that this is enough for now.
