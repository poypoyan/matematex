...
\begin{enumerate}
    ...
    \item \textbf{Non-well-founded} Ackermann Schema $\textsf{NWFAck}(V)$: Let $p$ be an \textit{unassigned} $(m+1)$-ary predicate. Let $$\phi(y, z_1, \ldots, z_m, z_{m+1}, \ldots, z_{m^2-m}, z_{m^2-m+1}, \ldots, z_{m^2}, a_1, \ldots, a_n)$$ be any unary first-order formula where all the free variables (all except $y$) are $\in V$ and $\ne V$, and there are $m$ instances of $p$ in $\phi$:
\begin{align*}
...
Now the Non-well-founded Ackermann Schema is formulated for easy contruction of non-well-founded sets (obviously), but it looks spooky and extremely complicated. Nevertheless, the intuition is actually simple:
...
Lastly, still in NWF Ackermann Schema, the motivation behind restricting to $m$ instances of $p$ in $\phi$ and the usage of $z_m \ldots z_{m^2}$ in $p$ in the final clause is also the above definition of ordered pair. We hope that this is enough.
